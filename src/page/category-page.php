...
<script type="text/javascript">
		...
		$j(document).ready(function() {
			...
			$j("#sl_category_patern").change(function() {
					$j("#sl_category_value").attr("readonly",false);
					if ($j(this).val() == <?php echo Salon_Category::TEXT; ?> ) {
						$j("#sl_category_value").val("");
						$j("#sl_category_value").attr("readonly",true);
					}
			});
		...
		function fnSelectRow(target_col) {
			...
			$j("#sl_category_value").val(htmlspecialchars_decode(setData['aoData'][position[0]]['_aData']['category_values']));	
		...
		function fnClickAddRow(operate) {
			if($j("#sl_category_patern").val() == <?php echo Salon_Category::TEXT; ?> ) {
				if ( ! checkItem("data_detail","sl_category_value") ) return false;
			}
			else {
			...
			$j.ajax({
					...
					data: {
						...
						"category_values":$j("#sl_category_value").val(),
	...
	<div id="data_detail" >
		...
		<textarea id="sl_category_value"  ></textarea>
