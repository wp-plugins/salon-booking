...
<?php //予約なしの実績登録以外では不要だが判定するほどのことではないので、そのまま ?>
		...
		function fnClickAddRow(operate) {
			...
			$j.ajax({
				 	type: "post",
					url:  "<?php echo get_bloginfo( 'wpurl' ); ?>/wp-admin/admin-ajax.php?action=sales",
					dataType : "json",
...
<?php //[TODO]salesに対するinsertなので判定をわけない ?>
	...
	<h2><?php _e('Performance Regist',SL_DOMAIN); ?>
	<?php  
			...
			<div id="input_aft">
					<div id="date_time_wrap" >
						...
						<div id="time_sel_wrap" >
							...
							<?php parent::echoTimeSelect("time_to_aft",$this->branch_datas['open_time'],$this->branch_datas['close_time'],$this->branch_datas['time_step']); ?>
						</div>
					</div>
